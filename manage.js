...
async function initParameters(){
  ...
  root.innerHTML=Object.entries(meta).map(([path,m])=>{const v=pathGet(current,path);return `<article class="param" data-path="${esc(path)}"><div class="param-head"><div><b>${esc(m.label)}</b><small>${esc(path)}</small></div><span>Défaut <strong>${m.default}</strong></span></div><div class="param-edit"><input type="number" value="${v}" min="${m.min}" max="${m.max}" step="${m.step}"><button class="apply">Appliquer</button><button class="secondary reset">Défaut</button></div><div class="effect"></div></article>`}).join('');
  function updateCard(card){const path=card.dataset.path,m=meta[path],v=Number(card.querySelector('input').value);let txt='Valeur par défaut : aucun changement par rapport à V14.';if(v>m.default)txt=m.higher;else if(v<m.default)txt=m.lower;card.querySelector('.effect').textContent=txt}
  ...
  const tech=document.getElementById('technicalReference');tech.innerHTML=`RSI : ${current.indicators.rsi_period} périodes · RVOL : ${current.indicators.rvol_period} périodes · Support : ${current.indicators.support_lookback} jours. Ces paramètres techniques restent verrouillés pour l'instant afin de ne pas modifier les formules pendant cette étape.`;
}
