...
async function initParameters(){
  ...
  function updateCard(card){const path=card.dataset.path,m=meta[path],v=Number(card.querySelector('input').value);let txt='Valeur par défaut : réglage de référence.';if(v>m.default)txt=m.higher;else if(v<m.default)txt=m.lower;card.querySelector('.effect').textContent=txt}
