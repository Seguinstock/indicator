(async()=>{
  ...
  try{
    ...
    chronological.forEach(day=>{
      const a=Number(day.average_return_pct);
    ...
    summary.innerHTML=`<div><b>Portefeuille théorique : 10 000 $</b> · Capital actuel <b>${money(capital)}</b> · Rendement total <b>${total>=0?'+':''}${total.toFixed(2)} %</b></div><div style="margin-top:5px"><b>${done.length}</b> journées complétées${avg===null?'':` · Moyenne quotidienne <b>${avg.toFixed(2)} %</b>`}</div><div class="hint" style="margin-top:5px">Chaque matin, tout le capital est réparti également entre les titres sélectionnés; tout est revendu à 17 h, puis le capital obtenu est réinvesti le jour ouvrable suivant.</div>`;
    ...
    box.innerHTML=newest.map((day,idx)=>{
      ...
      const rows=(day.picks||[]).map((p,i)=>{const rr=Number(p.return_pct);return `<tr><td>${i+1}</td><td><b>${p.symbol||''}</b></td><td>${Number(p.score||0).toFixed(1)}</td><td>${Number(p.risk||0).toFixed(1)}</td><td>${money(allocation)}</td><td>${p.start_price==null?'—':Number(p.start_price).toFixed(2)}</td><td>${p.end_price==null?'—':Number(p.end_price).toFixed(2)}</td><td>${Number.isFinite(rr)?`${rr>=0?'+':''}${rr.toFixed(2)} %`:'—'}</td></tr>`}).join('');
      return `<details class="stock" ${idx===0?'open':''}><summary><b>${day.date}</b> · ${day.count||0} titres · moyenne <b>${av}</b> · capital <b>${capitalText}</b></summary><div style="overflow:auto"><table style="width:100%;margin-top:10px"><thead><tr><th>#</th><th>Titre</th><th>Potentiel</th><th>Risque</th><th>Montant théorique</th><th>8 h</th><th>17 h</th><th>Rendement</th></tr></thead><tbody>${rows}</tbody></table></div></details>`;
    }).join('');
