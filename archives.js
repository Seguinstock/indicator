(async()=>{
  ...
  // Historical open-to-close benchmarks, using the same market mix as each archived top-12.
  // Sources checked 2026-09-17: S&P/TSX Composite and S&P 500 historical open/close data.
  const historicalBenchmarks={
    '2026-09-14':{tsx_composite_return_pct:0.279,sp500_return_pct:0.112,canada_picks:5,usa_picks:7,return_pct:0.182,method:'open_to_close_weighted_by_pick_market'},
    '2026-09-15':{tsx_composite_return_pct:-0.281,sp500_return_pct:-0.350,canada_picks:10,usa_picks:2,return_pct:-0.292,method:'open_to_close_weighted_by_pick_market'},
    '2026-09-16':{tsx_composite_return_pct:-0.517,sp500_return_pct:-0.650,canada_picks:7,usa_picks:5,return_pct:-0.573,method:'open_to_close_weighted_by_pick_market'}
  };
  ...
  try{
    ...
    summary.innerHTML=`<div><b>Portefeuille théorique : 10 000 $ le 14-09-26</b> · Stockindicator <b>${money(capital)}</b> · Marché comparable <b>${money(marketCapital)}</b> · Écart <b>${capitalGap>=0?'+':''}${money(capitalGap)}</b></div><div style="margin-top:5px">Rendement cumulé Stockindicator <b>${total>=0?'+':''}${total.toFixed(2)} %</b> · Marché <b>${marketTotal>=0?'+':''}${marketTotal.toFixed(2)} %</b> · Écart <b>${totalGap>=0?'+':''}${totalGap.toFixed(2)} pt</b>${annualized===null?'':` · Taux annualisé Stockindicator <b>${annualized>=0?'+':''}${annualized.toFixed(2)} %</b>`}</div><div style="margin-top:5px"><b>${done.length}</b> journées complétées · <b>${benchmarkDays}</b> avec comparatif marché${avg===null?'':` · Moyenne quotidienne Stockindicator <b>${avg.toFixed(2)} %</b>`}</div><div class="hint" style="margin-top:5px">Les deux portefeuilles partent de 10 000 $. Le comparatif marché compose, jour après jour, le S&P/TSX pour les titres canadiens et le S&P 500 pour les titres américains, pondérés selon le nombre de titres, sur la même fenêtre ouverture → clôture. Taux annualisé : extrapolation sur 252 séances, pas une projection. PO. = potentiel · RI. = risque · TI. = timing.</div>`;
    if(!days.length){box.textContent='Aucune journée archivée pour le moment.';return;}
    const observations=days.flatMap(day=>(day.picks||[]).filter(p=>p.return_pct!=null&&p.score!=null&&p.risk!=null&&p.timing!=null).map(p=>({...p,date:day.date})));
    const stat=v=>({n:v.length,avg:v.length?v.reduce((s,x)=>s+Number(x.return_pct),0)/v.length:null,win:v.length?100*v.filter(x=>Number(x.return_pct)>0).length/v.length:null});
    const bands=[['95+',x=>x.score>=95],['92-94.9',x=>x.score>=92&&x.score<95],['89-91.9',x=>x.score>=89&&x.score<92],['<89',x=>x.score<89]];
    const analysisRows=bands.map(([label,test])=>{const s=stat(observations.filter(test));return '<tr><td>'+label+'</td><td>'+s.n+'</td><td>'+(s.avg==null?'—':(s.avg>=0?'+':'')+s.avg.toFixed(2)+' %')+'</td><td>'+(s.win==null?'—':s.win.toFixed(1)+' %')+'</td></tr>'}).join('');
    const analysis=document.createElement('section');analysis.className='card';analysis.style.marginBottom='14px';
    analysis.innerHTML='<h2>Analyse de la formule</h2><div class="hint">Statistiques descriptives des archives; la formule de production demeure inchangée.</div><div style="overflow:auto;margin-top:8px"><table style="width:100%"><thead><tr><th>Score</th><th>N</th><th>Rendement moyen</th><th>% positif</th></tr></thead><tbody>'+analysisRows+'</tbody></table></div><div class="hint" style="margin-top:8px"><b>'+observations.length+'</b> observations exploitables. Les nouvelles sélections archivent aussi les composantes détaillées du modèle pour permettre une analyse plus fine à mesure que l’échantillon grandit.</div>';
    summary.parentElement.insertBefore(analysis,box);
