...
def sp500_pit_rows():
    if ASOF<pd.Timestamp('2021-01-01'):raise ValueError('Historical S&P 500 point-in-time mode is available from 2021 onward')
    import pitindex
    members=pitindex.get_constituents(ASOF.strftime('%Y-%m-%d'),index='sp500'); tickers=[]; seen=set()
    ...
    if not tickers:raise ValueError('No historical S&P 500 constituents found for requested date')
    return [{'symbol':s,'market':'XNYS','country':'US','enabled':'true'} for s in tickers]


def universe_rows():
    if UNIVERSE_MODE=='sp500_pit':return sp500_pit_rows(),'S&P 500 historique'
