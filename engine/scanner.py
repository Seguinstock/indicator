...
def macd_v14(close):
    ...
    mmomstd = sample_std(hd[-min(20,len(hd)):]); maccstd = sample_std(ha[-min(20,len(ha)):]); mhiststd = sample_std(hist[-min(20,len(hist)):])
...
def main():
    ...
    for x in results:
        ...
        y=dict(x); y['sell_timing_v14']=round(corrected,1); y['potential_v14']=round(potential,1); y['sell_signal']=sell_signal(corrected); y['score']=round(opportunity,1); y['filters']={}; y['passed']=True
        ranked[x['symbol']]=y
    sell_by_portfolio={}
